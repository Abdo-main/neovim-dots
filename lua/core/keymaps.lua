...
vim.keymap.set('n', '<leader>q', ':q<CR>', { desc = 'Close buffer/window' })

-- Escape from terminal mode back to normal mode
vim.keymap.set('t', '<Esc>', [[<C-\><C-n>]], { desc = 'Exit terminal mode' })

-- Toggle terminal visibility (hide but not kill)
vim.keymap.set('n', '<leader>`', function()
  ...
  if term_bufnr then
    -- if terminal is open in a window, hide it
    local wins = vim.fn.win_findbuf(term_bufnr)
    if #wins > 0 then
      vim.cmd 'hide'
    else
      vim.cmd('sb ' .. term_bufnr) -- reopen in a split
    end
  else
    vim.cmd 'split | term'
  end
